...
Para ubicar de forma explícita el aporte frente a la literatura, la Tabla~\ref{tab:related_comparison} contrasta cuatro trabajos representativos de ML sobre FAERS según su alcance, la granularidad de la tarea y la presencia de un control de fuga explícito.

\begin{table}[H]
\centering
\caption{Posicionamiento metodológico frente a trabajos representativos de aprendizaje automático sobre FAERS.}
\label{tab:related_comparison}
\small
\begin{tabular}{@{}p{3.6cm}p{3.0cm}p{3.2cm}p{2.6cm}@{}}
\toprule
\textbf{Trabajo} & \textbf{Alcance} & \textbf{Tarea (granularidad)} & \textbf{Control de fuga explícito} \\
\midrule
Oncología~\cite{oncoSAE2026} & Una clase terapéutica & Binaria (grave/no grave) & No \\
PD-1/PD-L1~\cite{pd1faers2025} & Inmunoterapia & Binaria (grave/no grave) & No \\
Detección automática~\cite{automatingADE2024} & General & Binaria (grave/no grave) & No \\
Ensamble de señales raras~\cite{ensembleRare2026} & General & Detección de RAR raras & No \\
\textbf{Este trabajo} & Trimestre completo & Ordinal, cinco niveles & Sí (dos capas) \\
\bottomrule
\end{tabular}
\end{table}

La comparación directa de valores de desempeño con estos trabajos no es metodológicamente válida: operan sobre tareas binarias y poblaciones específicas (una clase terapéutica), mientras que aquí se aborda un problema ordinal de cinco clases sobre el corpus completo de un trimestre y con neutralización de \textit{proxies} del desenlace. En ese contexto, los valores honestos obtenidos (\(F_1\) macro 0,517 y AUC macro 0,867) deben interpretarse como un techo deliberadamente más conservador que los AUC altos habituales en esquemas binarios, y no como un desempeño inferior: ambos miden tareas distintas. La contribución diferencial de este trabajo no es superar dichos valores, sino reportar métricas auditadas contra fuga de información sobre una tarea más granular.
...
Para construir el conjunto de modelado se utilizó como entrada el \textit{dataset} balanceado por \textit{Random Undersampling} (15.038 registros), que iguala el soporte entre los cinco niveles de severidad y mantiene un tamaño manejable. Se optó por submuestreo, y no por entrenar directamente sobre el corpus completo desbalanceado, por tres motivos: iguala el soporte de las clases sin inyectar muestras sintéticas en la base de datos, reduce el costo de la validación cruzada repetida sobre cuatro modelos, y evita que el sobremuestreo posterior deba expandir las clases minoritarias en factores extremos. Reconocemos como contrapartida que esta decisión limita el volumen efectivo de entrenamiento; entrenar sobre una base mayor mediante un submuestreo menos agresivo o reponderación sensible al costo sobre el corpus completo se plantea como mejora directa de trabajo futuro (Sección~\ref{sec:future}). Sobre esa base se aplicó además SMOTE como sobremuestreo, pero exclusivamente sobre el conjunto de entrenamiento y, durante la validación cruzada, encapsulado dentro de un \textit{pipeline}, de modo que cada pliegue de validación nunca observa muestras sintéticas. La etapa de preparación selecciona dinámicamente el \textit{dataset} balanceado disponible, por lo que la técnica de balanceo de base es configurable sin alterar el resto del flujo.
