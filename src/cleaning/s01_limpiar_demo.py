...
Usa PySpark si Java está disponible, o realiza un fallback en Pandas nativo.
...
def obtener_spark_session() -> SparkSession:
    ...
    if not PYSPARK_AVAILABLE:
        raise RuntimeError("PySpark no está instalado.")
...
def limpiar_demo_pandas() -> dict:
    """Fallback en Pandas para limpiar la tabla DEMO si Spark/Java no están disponibles."""
    print("  [FALLBACK] Usando Pandas para procesar la tabla DEMO...")
    import pandas as pd
    import pyarrow as pa
    import pyarrow.parquet as pq

    log = {}
    raw_path = RAW_DATA / "DEMO25Q4.txt"
    ...
    df = pd.read_csv(
        ...
        sep=FAERS_SEP,
        encoding=encoding,
        dtype=str, # Leer todo como str para evitar problemas y luego convertir
    ...
    df = df[df["primaryid"].notna() & (df["primaryid"].str.strip() != "")]
    filas_con_id = len(df)
    ...
    sexo_validos = set(RULES_DEMO["sexo_validos"])

    # Normalizar sex
    df["sex"] = df["sex"].str.strip()
    mask_invalido = df["sex"].isna() | (df["sex"] == "") | (~df["sex"].isin(sexo_validos))
    df.loc[mask_invalido, "sex"] = sexo_default

    normalizados_sex = (df["sex"] == sexo_default).sum()
    ...
    # Fuera de rango a nulo
    mask_fuera_rango = (df["age"] < edad_min) | (df["age"] > edad_max) | df["age"].isna()
    df.loc[mask_fuera_rango, "age"] = None
    ...
    if "reporter_country" in df.columns:
        ...
        paises_no_std = (df["reporter_country"].str.len() != 2).sum()
    ...
    for col in df.columns:
        # Consideramos nulo tanto NA como vacío
        null_c = df[col].isna().sum() + (df[col] == "").sum()
...
def main():
    print("\n  [s01] Limpieza DEMO (PySpark/Pandas)")
    print("  " + "-" * 56)
    
    # Intentar ejecutar con Spark. Si falla por falta de Java o PySpark, ir al fallback
    spark = None
    try:
        if not PYSPARK_AVAILABLE:
            raise RuntimeError("PySpark no está importable.")
        ...
        # Convertir a Pandas y guardar nativo
        ...
        # Capturamos fallos de Spark o Java y corremos en Pandas
        print(f"\n  [INFO] Spark/Java no disponible o falló con: {e}")
        try:
            limpiar_demo_pandas()
        except Exception as e_pandas:
            print("\n" + "!" * 80)
            print("  [ERROR CRÍTICO] Ambos pipelines (Spark y Pandas fallback) fallaron:")
            print("!" * 80 + "\n")
