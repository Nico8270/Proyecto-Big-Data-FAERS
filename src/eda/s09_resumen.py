...
def decidir_accion(df_cols: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in df_cols.iterrows():
        ...
        if row.es_pk:
            ...
            accion, justif = "eliminar_columna", f"Nulos altos ({pct}%),不在 es predictiva"
...
def imprimir_balanceo(df_bal: pd.DataFrame):
    ...
    for _, row in df_bal.iterrows():
        print_fila_columnas(
            [row.clase, row.descripcion, f"{row.count:,}",
             f"{row.porcentaje:.1f}%", f"{row.ratio}x", row.estrategia],
            AN_BAL)
    print(f"  {'─'*(sum(AN_BAL)+10)}")
    may, min_ = df_bal["count"].max(), df_bal["count"].min()
    print(f"  Ratio desbalance : {may/min_:.1f}x  (clase 1 vs clase {df_bal['clase'].iloc[-1]})")
    print(f"  Metrica recomendada : Macro-F1 (no Accuracy)")
...
def main():
    ...
    print(f"  {len(df_colas)} columnas analizadas en {df_cols['tabla'].nunique()} tablas")
